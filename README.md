...
- 우리가 그동안에 검색했던 방법은 **일치**. Table에 있는 id와 일치하는 것. 

  이 컬럼은 인덱싱이 되어 있기 때문에 검색 속도가 매우 빠르고 항상 고유한 값을 가진다.

  Ex. 갑자기 중간거를 삭제했다고 해서 채워지거나 하지X. 무조건 증가
...
- Table에 있는 id값으로 해결하지 못하는 경우?
  ...
  - `find_by`의 특징: **1개만 검색**됨, 일치하는 값이 없는 경우 `nil`

- 추가적인 검색방법: `Model.where(컬럼명: 검색어값)`

  - `User.where(user_name: "Hello")`

  - `where`의 특징: **검색결과가 여러개 가능**(일치되는 모든 값을 찾아줌) 결과값이 배열 형태(단, 배열은 X). 

    일치하는 값이 없는 경우에도 빈 배열이 나옴. 
...
- 포함?
  ...
  ```ruby
  ## rails c 로 확인함! ##
  hanullllje:~/daum_cafe_app (master) $ rails c
  Running via Spring preloader in process 65995
  Loading development environment (Rails 5.0.7)
  2.3.4 :002 > u = User.find_by_user_id("xzcv")
    User Load (0.3ms)  SELECT  "users".* FROM "users" WHERE "users"."user_id" = ? LIMIT ?  [["user_id", "xzcv"], ["LIMIT", 1]]
   => nil 
  2.3.4 :003 > User.find_by_user_id("test")
    User Load (0.3ms)  SELECT  "users".* FROM "users" WHERE "users"."user_id" = ? LIMIT ?  [["user_id", "test"], ["LIMIT", 1]]
   => #<User id: 3, user_id: "test", password: "1234", ip_address: "222.107.238.10"> 
  2.3.4 :004 > User.where(user_id: "zxcv")
    User Load (0.4ms)  SELECT "users".* FROM "users" WHERE "users"."user_id" = ?  [["user_id", "zxcv"]]
   => #<ActiveRecord::Relation []> 
  2.3.4 :005 > User.where(user_id: "zxcv").nil?
   => false 
  2.3.4 :006 > User.find_by(user_id: "zxcv")
    User Load (0.3ms)  SELECT  "users".* FROM "users" WHERE "users"."user_id" = ? LIMIT ?  [["user_id", "zxcv"], ["LIMIT", 1]]
   => nil 
  2.3.4 :007 > User.where(user_id: "zxcv")
    User Load (0.2ms)  SELECT "users".* FROM "users" WHERE "users"."user_id" = ?  [["user_id", "zxcv"]]
   => #<ActiveRecord::Relation []> 
  2.3.4 :008 > User.where("user_id LIKE?", "%te%")
    User Load (0.4ms)  SELECT "users".* FROM "users" WHERE (user_id LIKE'%te%')
   => #<ActiveRecord::Relation [#<User id: 3, user_id: "test", password: "1234", ip_address: "222.107.238.10">, #<User id: 4, user_id: "test", password: "12345", ip_address: "222.107.238.10">]>
  2.3.4 :009 > u =User.where(user_id: "zxcv")                                                                                                                                      
    User Load (0.2ms)  SELECT "users".* FROM "users" WHERE "users"."user_id" = ?  [["user_id", "zxcv"]]
   => #<ActiveRecord::Relation []> 
  2.3.4 :010 > u.empty?
   => true 
